...
// Flush hairline grid: bottom hairlines everywhere but the last row, right
// hairline on left-column cells at sm and up
const CELL_BORDERS = [
...
export function WhatWeBuy() {
  return (
    <section id="what-we-buy" className="bg-background border-b border-border scroll-mt-28 md:scroll-mt-24 snap-start">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
        ...
        <div className="grid sm:grid-cols-2 border border-border">
          {CRITERIA.map((item, i) => (
            <div
              key={item.index}
              className={cn('p-6 sm:p-8 border-border', CELL_BORDERS[i])}
            >
