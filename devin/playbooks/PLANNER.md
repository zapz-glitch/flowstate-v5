...
record it as `BASE:` in the packet.
...
Pushing the spec branch IS the dispatch — a `github:push` automation fires the
Builder on any `devin/*` push whose head commit message starts with `spec `.
That is why the commit message convention `spec NNN: <title>` is load-bearing:
do not amend it, do not squash it into other commits, and never push
